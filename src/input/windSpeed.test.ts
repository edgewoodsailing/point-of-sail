...
describe("the wind speed range", () => {
   ...
   * six files.
   */
  it("runs 0 to 25 knots in whole-knot steps", () => {
    expect(WIND_SPEED_KT.min).toBe(0);
    expect(WIND_SPEED_KT.max).toBe(25);
   ...
   * that never went through a conversion at all. 25 kt is 12.86 m/s; a slider
   * whose reading was passed straight through would set 25 m/s, which is 48.6 kt
   * and a hurricane.
   */
  it("hands the model metres per second, not knots", () => {
    expect(windSpeedFromKnots(25)).toBeCloseTo(12.8611, 4);
    expect(windSpeedFromKnots(25)).toBeCloseTo(knotsToMetersPerSecond(25), 12);
    // The mistake this rules out, stated so it cannot be read back in.
    expect(windSpeedFromKnots(25)).not.toBeCloseTo(25, 1);
  });
  ...
  it("clamps a reading outside the range, at both ends", () => {
    ...
    expect(metersPerSecondToKnots(windSpeedFromKnots(40))).toBeCloseTo(25, 9);
    ...
    expect(metersPerSecondToKnots(windSpeedFromKnots(25))).toBeCloseTo(25, 9);
  });
});
...
describe("what the slider shows", () => {
  ...
  it("reports an out-of-range wind at the end it exceeds", () => {
    expect(knotsFromWindSpeed(knotsToMetersPerSecond(40))).toBe(25);
   ...
   * clamping down to it. Worth its own case because the two arrive at 25 by
  ...
  it("rounds up to the top of the range rather than stopping short of it", () => {
    expect(knotsFromWindSpeed(knotsToMetersPerSecond(24.6))).toBe(25);
    expect(knotsFromWindSpeed(knotsToMetersPerSecond(24.4))).toBe(24);
  });
});
